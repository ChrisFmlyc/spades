...
1. **Preflight** — before running any git command:
   - If `~/.spade` does not exist, follow "If ~/.spade does not exist"
     below and stop.
   - If `~/.spade` exists but has no `.git/`, follow "If ~/.spade is not
     a git repo" below and stop.
   - Otherwise, run the following using the Bash tool to see what is new:
...
- **`/spade-learn`** — capture learnings that future Plans will surface
  automatically. See the `spade-learn` skill.
