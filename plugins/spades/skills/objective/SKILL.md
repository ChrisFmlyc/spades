...
§ .spades/ Local Layout before running. The schema below mirrors that
contract.
...
`/spades:close O-<slug>` (per skill isolation, this skill never invokes
close inline).
...
`.spades/objectives/O-<slug>.md` — the AI-readable source of truth and the
canonical record. In HTML mode, **additionally** render via the sibling
`${CLAUDE_PLUGIN_ROOT}/skills/objective/template.html` and write
`.spades/objectives/O-<slug>.html` for the human's view, then auto-open via
the OPEN_CMD prelude. HTML mode is additive — the `.md` always exists; the
`.html` is added in HTML mode.
...
1. **Confirm setup.** If `.spades/config` is missing, abort and suggest
   `/spades:setup` first.
2. **Confirm active project.** Read `project:` from `.spades/config`. If
   missing, abort and suggest `/spades:newproject` to create one.
...
4. **No INTENT hard-gate.** Unlike `/spades:scope`, this skill does **not**
   hard-gate on `INTENT.md`. An Objective is itself a strategy-level
...
Derive the slug from the title, using the same rule as `/spades:newproject`
and `/spades:scope`:
...
**Collision check.** If `.spades/objectives/O-<slug>.md` already exists,
this is an Edit operation — switch modes and warn the human. When
...
**Read `review_format:` from `.spades/config` and branch on file format.**
Step 4 MUST write the Objective `.md` before exiting — never print the
record to the CLI only, and never paste the body to the CLI for approval
...
Write `.spades/objectives/O-<slug>.md` with this exact shape:
...
|-----------|---------------|---------|
| `worker-file-objective` | `.spades/objectives/O-<slug>.md` — the canonical `.md` (Step 4.A shape), written **without** `linear_milestone_id` / `linear_issue_id`; the coordinator injects them post-dispatch. | `{ status: ok }` (or `fail` + `error`) |
| `worker-html-objective` *(only when `review_format: html`)* | `.spades/objectives/O-<slug>.html` — see Step 4.B. | `{ status: ok, path, opened }` |
...
Next:
  /spades:close O-<slug>   — mark complete when the team lead judges the
                             objective reached (or --abandon to walk away)
```
...
- **Description reads like a task, not an objective.** Push back: an
  ...
  Suggest `/spades:scope` if the human is actually describing deliverable
  work.
- **Human wants to attach scopes.** Explain that Objectives and Scopes are
  ...
  `strategy_link:` to `O-<slug>` via `/spades:scope` (Edit mode); it is
  purely documentary.
