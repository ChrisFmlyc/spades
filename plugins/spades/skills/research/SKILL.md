...
context — for example, mid-`/spades:plan` on `S-add-ai-helper-bot`),
...
2. **Identify the Scope context.** Was `--scope <id>` passed? Is the
   ...
   the human is mid-`/spades:plan`)? If yes, capture the Scope ID for
...
rejected), follow the same precedent as `/spades:plan`'s Linear
Integration:
...
4. Do **not** auto-create a fallback `.spades/research/<id>.md` file —
...
- It does not invoke `/spades:review`. Research is fact-finding from
  outside; review is opinion on our own work. They're different.
- It does not capture findings as `/spades:learn` learnings
  automatically. If the report uncovers something worth carrying
  forward to future Plans, the human can invoke `/spades:learn`
  separately.
