...
# SPADES Framework — learnings lint
#
# Validates the shape of files under .spades/learnings/:
#
...
#      candidates for `/spades-learn --refresh`.
#
# Does NOT recurse into .spades/learnings/private/: that directory is
...
if [ ! -d "$LEARN_DIR" ]; then
    echo "lint-learnings: no .spades/learnings/ directory; nothing to validate"
    exit 0
fi
...
for f in "${files[@]}"; do
    ...
    # content and must live under .spades/learnings/private/. If it lands in
    ...
    case "$f" in
        ...
        *)
            if [ "$public_safe" = "false" ]; then
                echo "  FAIL: $rel has public_safe=false but is NOT under .spades/learnings/private/ — move it to private/ or change public_safe to true"
                fail=$((fail + 1))
                continue
            fi
            ;;
    ...
    if [ "$status" = "active" ]; then
        ...
        if [ "$created_epoch" -gt 0 ]; then
            ...
            if [ "$age" -gt "$one_eighty_days" ]; then
                days=$((age / 86400))
                echo "  warn: $rel is $days days old and still active — candidate for /spades-learn --refresh"
                warn=$((warn + 1))
            fi
        fi
    fi
