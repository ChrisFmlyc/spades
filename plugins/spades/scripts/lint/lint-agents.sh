...
# SPADES Framework — agent (subagent) frontmatter lint
#
# Walks every agents/*.md in the repo and asserts the
# frontmatter parses cleanly and carries the required fields for a
# SPADES-defined persona subagent.
#
# Required fields:
#   - name         — e.g. review-scope-guardian
...
#
# Required SPADES-specific fields:
...
#   2  repo has no agents/ directory (fine for pre-v1.1.0
...
if [ ! -d "$AGENTS_DIR" ]; then
    echo "lint-agents: no agents/ directory; nothing to validate"
    exit 0
fi
...
if [ "${#files[@]}" -eq 0 ]; then
    echo "lint-agents: agents/ exists but is empty; nothing to validate"
    exit 0
fi
