...
§ ID Format, § .spades-anywhere/ Local Layout, and § Backend Interface below.
...
- Stored at: `.spades-anywhere/objectives/O-<description-slug>.md`.
...
### `.spades-anywhere/objectives/O-<slug>.md` frontmatter
...
- An Objective is just `.spades-anywhere/objectives/O-<slug>.md` — no
  milestone, no issue; `status: complete` is its completion signal
...
modes), `/spades-anywhere:objective` (create and edit modes — no new
Objective under an abandoned/archived Project), `/spades-anywhere:plan`,
`/spades-anywhere:approve`, `/spades-anywhere:do`,
`/spades-anywhere:evaluate`, `/spades-anywhere:ship`, and
`/spades-anywhere:close` on the **Pass** route.
...
`/spades-anywhere:close O-… ` to reconcile the local record.
