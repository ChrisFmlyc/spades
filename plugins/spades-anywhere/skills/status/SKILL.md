...
1. Glob `.spades-anywhere/scopes/*.md` for active scopes (not `done`,
...
2. For dependency information, fall back to the local `.spades-anywhere/plans/`
...
- A Plan with `delivery: undecided` → `Re-run /spades-anywhere:approve P-… (routing not set)`
...
3. Write the rendered HTML to `.spades-anywhere/.tmp/status.html` (creating
   `.spades-anywhere/.tmp/` if missing — it is auto-gitignored by
